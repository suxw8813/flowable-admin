<template>
  <el-menu :default-active="$store.state.app.checkRouter" class="el-menu-vertical-demo el-aside" :collapse="isCollapse"
           background-color="#2b2b38" text-color="#ffffff" unique-opened router>
    <el-submenu v-for="item in menu" :key="item.id" :index="item.name">
      ...
      <el-menu-item-group>
        <el-menu-item v-for="childItem in item.children" :key="childItem.id"
                      :index="childItem.url ? childItem.url : '#' " v-if="childItem.type === 'cd'"><span></span>{{childItem.name}}
        </el-menu-item>
      </el-menu-item-group>
      <el-submenu v-for="childItem in item.children" :key="childItem.id"
                  :index="childItem.url ? childItem.url + '/' + childItem.id : '#'" v-if="childItem.type === 'ml'">
        <template slot="title">
          <i class="el-icon-location"></i>
